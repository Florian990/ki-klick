...
import { GraduationCap, Users, Briefcase, XCircle, Check, ChevronRight, ChevronLeft, Euro, Home, MapPin, Clock, BookOpen, Wallet } from "lucide-react";
...
const questions: QuizQuestion[] = [
  {
    ...
    answers: [
      { text: "Schüler/in", icon: <BookOpen className="h-6 w-6 sm:h-8 sm:w-8" />, disqualify: true },
  ...
  {
    id: 5,
    question: "Warum möchtest du dir ein zweites Standbein aufbauen?",
    answers: [
      { text: "Einkommen über 2.500€", icon: <Euro className="h-6 w-6 sm:h-8 sm:w-8" /> },
      { text: "Mehr Zeit für Familie und Freunde", icon: <Users className="h-6 w-6 sm:h-8 sm:w-8" /> },
      { text: "Arbeiten von zu Hause", icon: <Home className="h-6 w-6 sm:h-8 sm:w-8" /> },
...
export default function Quiz({ onComplete, onDisqualify }: QuizProps) {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-2 sm:px-4">
      ...
      <div className={`grid gap-2 sm:gap-3 md:gap-4 ${currentQuestion.answers.length === 6 ? 'grid-cols-2 sm:grid-cols-3' : currentQuestion.answers.length === 4 ? 'grid-cols-2 lg:grid-cols-4' : currentQuestion.answers.length === 3 ? 'grid-cols-1 sm:grid-cols-3' : 'grid-cols-1 sm:grid-cols-2'}`}>
